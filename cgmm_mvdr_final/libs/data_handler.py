...
class SegmentSpecReader(WaveReader):
    """
    WaveReader with segments and apply stft to the single/multi-channel signals
    """
    def __init__(self, wav_scp, normalize=True, **kwargs):
        super(SegmentSpecReader, self).__init__(wav_scp, normalize=normalize)
        self.stft_kwargs = kwargs
        self.frame_hop = self.stft_kwargs['frame_hop']
        self.frame_len = self.stft_kwargs['frame_len']
        self.signal_left = []

    def _load(self, key, beg=None, end=None):
        # get wave samples
        samps = super().read(key, beg=beg, end=end)

        # append the left signal
        if len(self.signal_left) > 0:
            samps = np.concatenate((self.signal_left.pop(), samps), axis=1)

        # apply stft to single.multi-channel signals
        if samps.ndim == 1:
            stft_mat = forward_stft(samps, **self.stft_kwargs)
        else:
            N, _ = samps.shape
        ...
        num_frames = stft_mat.shape[-1]
        tail = (num_frames - 1)* self.frame_hop + self.frame_len
        start_index = tail - self.frame_len + self.frame_hop

        # 当每次block处理的信号不能构成整数帧时
        # 由于num_frame = floor((samps_len-frame_len)/frame_shift)，所以会漏掉一部分数据
        # 将漏掉的数据保存在signal_left中，用于下一次的处理
        # 此外为了各个block之间的数据没有
        self.signal_left.append(samps[:, start_index:])
        return stft_mat
